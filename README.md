...
SoloFlow 是一款专为个人设计的轻量本地工作流工具。无论是个人计划、学习进度、工作事项还是旅行准备，都可以用它理清「先做什么、后做什么」，让每一步有序推进。

SoloFlow 以事项先后关系为主线，所有高亮的事项都是当前阶段可执行的行动，并根据依赖关系逐步解锁。不需要建立复杂的标签体系，也不需要搭建多层的任务层级，通过项目状态（未开始 / 进行中 / 已完成 / 停滞）和甘特图时间线来呈现事项的全貌。你可以直观地看到每件事在时间轴上的位置，以及项目整体的进度。
...
**Todo List 告诉你有哪些事情，SoloFlow 帮你理清这些事情应该先做什么。**

很多事情天然有先后顺序：先做事前申请才能报销，先订好行程才能订酒店，先学完基础才能进入进阶内容。在 SoloFlow 中：

- 为每个事项设置「前置事项」，前置未完成时后续事项自动锁定，随时知道当前该做什么
- 前置完成后自动解锁，流程自然向前推进
- 配合甘特图，回顾每个事项的实际完成时间与整体进度
...
- 正在准备考试、考证或学习一门新技能
- 正在进行个人项目创作（写作、视频、副业），想清楚“先做什么才能让下一步动起来”
- 同时处理多个生活或工作事项（旅行、搬家、考证、求职），需要一个视角看到整体进度
- 对复杂项目管理软件感到疲惫，不想花时间配置系统，只想打开即用、轻量顺手
